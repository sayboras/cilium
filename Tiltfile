...
local_resource('cilium-operator-compile', 'make -j $(nproc) operator', deps=source['operator_source'],
               ignore=source['ignore_files'],
               labels=['build'])
...
# Docker build
docker_build(
    'cilium', '.', dockerfile='images/cilium/Dockerfile.debug',
    only=source['cilium_source'])

docker_build(
    'operator-generic-ci',
    '.',
    dockerfile='images/operator/Dockerfile.debug',
    build_args={
