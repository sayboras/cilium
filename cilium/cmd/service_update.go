...
func updateService(cmd *cobra.Command, args []string) {
	...
	case id == 0 && frontend == "" && len(backends) != 0:
		...
		// of the API is to update backend states for service(s) selecting those
		// backends.
		if len(backends) == 0 || len(backendStates) == 0 {
			Fatalf("Cannot update backend states with empty backends or backend-states")
		}
