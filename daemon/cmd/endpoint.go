...
func (d *Daemon) fetchK8sLabelsAndAnnotations(nsName, podName string) ([]types.ContainerPort, labels.Labels, labels.Labels, map[string]string, error) {
	...
	return containerPorts, identityLabels, infoLabels, annotations, nil
}
...
func (d *Daemon) createEndpoint(ctx context.Context, epTemplate *models.EndpointChangeRequest) (*endpoint.Endpoint, int, error) {
	...
	if ep.K8sNamespaceAndPodNameIsSet() && k8s.IsEnabled() {
		cp, identityLabels, info, _, err := d.fetchK8sLabelsAndAnnotations(ep.K8sNamespace, ep.K8sPodName)
		if err != nil {
