...
func Test_getBackendServices(t *testing.T) {
	...
	assert.Equal(t, []*v2alpha1.Service{
		{
			Name:      "dummy-backend",
...
func Test_getRouteConfigurationResource(t *testing.T) {
	...
	require.Equal(t, "/dummy-path", routeConfig.VirtualHosts[0].Routes[0].Match.GetPrefix())
	require.Equal(t, "dummy-namespace/dummy-backend", routeConfig.VirtualHosts[0].Routes[0].GetRoute().GetCluster())

	require.Equal(t, "/another-dummy-path", routeConfig.VirtualHosts[0].Routes[1].Match.GetPrefix())
	require.Equal(t, "dummy-namespace/another-dummy-backend", routeConfig.VirtualHosts[0].Routes[1].GetRoute().GetCluster())
...
func Test_getClusterResources(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "dummy-namespace/dummy-backend", cluster1.Name)

	cluster2 := &envoy_config_cluster_v3.Cluster{}
	err = proto.Unmarshal(res[1].Value, cluster2)
	require.NoError(t, err)
	require.Equal(t, "dummy-namespace/another-dummy-backend", cluster2.Name)
}
