...
func (ic *IngressController) handleIngressDeletedEvent(event ingressDeletedEvent) error {
	...
	log.WithField("ingress", event.ingress.Name).Debug("Deleting CiliumEnvoyConfig for ingress")
...
func (ic *IngressController) handleIngressServiceUpdatedEvent(ingressServiceUpdated ingressServiceUpdatedEvent) error {
	...
	if err != nil {
		log.WithError(err).WithField("service", serviceKey).Warn("Failed to lookup ingress for service")
		return err
	}
	scopedLog := log.WithField("ingress", ingressKey).WithField("service", serviceKey).
...
func (ic *IngressController) handleEvent(event interface{}) error {
	...
	case ingressAddedEvent:
		log.WithField("ingress", ev.ingress.Name).Debug("Handling ingress added event")
		err = ic.handleIngressAddedEvent(ev)
	case ingressUpdatedEvent:
		log.WithField("ingress", ev.newIngress.Name).Debug("handling ingress updated event")
		err = ic.handleIngressUpdatedEvent(ev)
	case ingressDeletedEvent:
		log.WithField("ingress", ev.ingress.Name).Debug("Handling ingress deleted event")
		err = ic.handleIngressDeletedEvent(ev)
	case ingressServiceUpdatedEvent:
		log.WithField("service", ev.ingressService.Name).Debug("Handling ingress service updated event")
...
func (ic *IngressController) createLoadBalancer(ingress *slim_networkingv1.Ingress) error {
	...
	if exists {
		...
		// controller.
		log.WithField("service", svcKey).Debug("Service already exists. Continuing...")
		return nil
	}
	...
	log.WithField("service", svcKey).Debug("Created Service for Ingress")
	return nil
}
...
func (ic *IngressController) createEndpoints(ingress *slim_networkingv1.Ingress) error {
	...
	if exists {
		...
		// controller.
		log.WithField("endpoints", key).Debug("Endpoints already exists. Continuing...")
		return nil
	}
	...
	log.WithField("endpoints", key).Debug("Created Endpoints for Ingress")
	return nil
}
...
func (ic *IngressController) createEnvoyConfig(ingress *slim_networkingv1.Ingress) error {
	...
	if exists {
		if desired.DeepEqual(existingEnvoyConfig) {
			log.WithField("cilium-envoy-config", key).Debug("No change for existing CiliumEnvoyConfig")
			return nil
		}
		// Update existing CEC
		...
		log.WithField("ingress", ingress.Name).Debug("Updated CiliumEnvoyConfig for ingress")
		return nil
	}
	...
	log.WithField("ingress", ingress.Name).Debug("Created CiliumEnvoyConfig for ingress")
	return nil
}
...
func (ic *IngressController) deleteCiliumEnvoyConfig(ingress *slim_networkingv1.Ingress) error {
	...
	if !exists {
		log.WithField("cilium-envoy-config", resourceName).Debug("CiliumEnvoyConfig already deleted. Continuing...")
		return nil
	}
	...
	log.WithField("cilium-envoy-config", resourceName).Debug("Deleted CiliumEnvoyConfig")
	return nil
}
